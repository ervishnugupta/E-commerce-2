...
const Nav = () => {
  ...
  const [auth, setAuth] = useState(!!localStorage.getItem('login')); // ✅ Ensures correct initial state
  const navigate = useNavigate();
  const { total_items } = useCartContext();

  useEffect(() => {
    const handleStorageChange = () => {
      setAuth(!!localStorage.getItem('login')); // ✅ Updates state on login/logout
    };
  ...
  const handleLogout = () => {
    ...
    setAuth(false); // ✅ Instantly updates state
    navigate('/login');
  };
